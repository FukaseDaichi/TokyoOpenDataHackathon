...
未収録区・未収録フィールドがあってもよく、ローダー `src/data/policies.ts` の `loadWardProfile()` は未収録区に `null` を返す。区章画像 `public/emblems/{slug}.svg` はWikimedia Commonsから手動配置したもので、同じく生成パイプラインの対象外である。区詳細ページは画像読み込み失敗時に `onError` で非表示にする。取得URL・取得日・ライセンス根拠はリポジトリ内に保持していないため、権利情報の再検証性は [07-risks-and-concerns.md](07-risks-and-concerns.md) の懸念事項とする。
...
- 区章: 採用時にWikimedia Commonsの配布ページと自治体章の扱いを確認する。取得URL、取得日、ライセンス根拠を別途記録し、根拠が不明な画像は使わない。画像取得に失敗しても区詳細ページの他の内容に影響しないよう、`<img onError>` で非表示にする実装（`src/ui/pages/WardPage.tsx`）を維持する。
