...
| `income_per_taxpayer` | 納税義務者1人当たり課税対象所得（千円） | 総務省「市町村税課税状況等の調」（令和6年度）を東京都総務局行政部が特別区分に再集計した第9表（`tokyo_r06_dai9hyo.xlsx`）。課税標準額の12段階（10万円以下～1億円超）を区ごとに合算し、課税標準額の合計 ÷ 納税義務者数（所得割）で算出する |
