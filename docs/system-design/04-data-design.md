...
| `waiting_children` | 待機児童数（人） | こども家庭庁「保育所等関連状況取りまとめ（令和7年4月1日）」（参考）資料1～6（`cfa_hoiku_r7_shiryo.xlsx`）の資料6-1・資料6-2。資料6は令和6年・令和7年ともにゼロの市区町村を掲載しない様式のため、未掲載の区は待機児童数ゼロとして扱う。資料6から拾った東京都全市区町村の合計が資料4「全国待機児童マップ」の東京都計と一致することをassertで検証してから区へ割り当てる |
