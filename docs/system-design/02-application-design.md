...
- 保存された結果区コードとURLの区が一致する場合: 自分の診断結果として、区ベクトルとの重ね描き、校正結果を先頭にした上位3区、類似度、X共有リンク、画面内シェアカードを表示する。
- 保存値がない場合、または結果区コードとURLが一致しない場合: 共有された結果の受け手として、URLが示す区と診断開始導線を表示する。
