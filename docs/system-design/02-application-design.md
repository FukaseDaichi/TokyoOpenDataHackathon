...
`ResultPage` はURLのslugから表示対象の区を決め、マウント後に `sessionStorage` の診断ベクトルと結果区コードを読む。

- 保存された結果区コードとURLの区が一致する場合: 自分の診断結果として、区別OGP画像に続けて次の診断者向けセクションを順に表示する。
  1. あなたのタイプ: 利用者ベクトルから導いたタイプ名と説明文（`src/lib/personaType.ts`）、区のレーダーに利用者ベクトルを破線で重ねたオーバーレイ表示と凡例
  2. なぜ相性がいいの？: 5軸それぞれの利用者・区の位置を示す比較バー。一致軸（`selectMatchedAxes`）のうち軸差0.5以下のものに「ここが一致！」バッジとハイライトを付け、一致軸1位のAI執筆相性文（`src/data/affinity.ts`、「AIによる相性解説」バッジ付き）を表示する
  3. 実はこんな街: 一致軸2つに対応する基本指標のデータカード（実値と23区順位、`statLabelForAxis` で軸→指標を解決）、一致軸にタグが合う区の政策1件（`pickPolicyForAxes`、出典リンク付き）、区詳細ページへの導線
  4. キャラクター設定理由、区単体のレーダーと基本7指標（受け手と共通）
  5. X共有リンク、校正結果区を除き利用者ベクトルとの未校正距離が近い他区3件の「相性ランキング」
- 保存値がない場合、または結果区コードとURLの区が一致しない場合: 共有された結果の受け手として、URLが示す区のOGP画像、キャラクター設定理由、区単体のレーダーと基本7指標、診断開始導線を表示する。利用者ベクトルがないため、あなたのタイプ・相性理由・実はこんな街・X共有リンク・相性ランキングは表示しない。
...
| `ResultPage` | 診断セッションの有無に応じた結果表示（タイプ・相性理由・データカード・政策の診断者向けセクションを含む） |
...
`src/ui/WardDetail.tsx` と `ShareCard.tsx` の `ShareCard` コンポーネントは現行ルートから参照されていない。`ResultPage` が利用するのは `src/ui/share.ts` の `xShareText()` / `xShareUrl()` だけである。未使用コードとしての扱いは [07-risks-and-concerns.md](07-risks-and-concerns.md) に記載する。
