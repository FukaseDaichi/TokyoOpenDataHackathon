...
URLには区slugだけが含まれ、回答や5軸ベクトルは含まれない。このため共有先では相性ランキングや利用者ベクトルを再現しない。OGPの公開パスは `/og/{slug}.png` である。

セッションには結果区コードを保存していないため、同じタブで別区の結果URLを開いても保存済みベクトルが使われる。この整合性上の懸念は [07-risks-and-concerns.md](07-risks-and-concerns.md) に記載する。

## 5. 区詳細ページ

`WardPage` は区を知るための図鑑本編であり、位置、5軸、根拠統計、政策、プロフィール、同系統区を1ページにまとめる。

- 位置: 23区境界データを使う3D地図。WebGLを使えない場合は同じ区を強調する2D SVG地図
- レーダー: 正規化済み5軸
- 統計バー: 基本7指標と、地価、外国人人口比率、平均所得、人口千人当たり刑法犯認知件数、待機児童数のうちデータが存在する項目
- 政策: 手動キュレーションした区の政策概要と外部出典リンク
- プロフィール: 区章、人口、面積、収録済みの花・木・鳥
- 同系統のなかま: k-meansで同じ数値ラベルになった区へのリンク
- 出典: 基本指標、詳細指標、行政区域データのスナップショット出典
