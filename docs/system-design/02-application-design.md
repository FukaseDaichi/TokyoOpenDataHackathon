...
- 保存された結果区コードとURLの区が一致する場合: 自分の診断結果として、区別OGP画像、X共有リンク、キャラクター設定理由、区単体のレーダーと基本7指標を表示する。さらに校正結果区を除き、利用者ベクトルとの未校正距離が近い他区3件を「相性ランキング」として表示する。
- 保存値がない場合、または結果区コードとURLの区が一致しない場合: 共有された結果の受け手として、URLが示す区のOGP画像、キャラクター設定理由、区単体のレーダーと基本7指標、診断開始導線を表示する。利用者ベクトルがないため、X共有リンクと相性ランキングは表示しない。
