...
| `Diagnosis` | `diagnosisFlow.ts` のフェーズ状態機械に従った質問進行・ページめくり演出・回答の収集、最終問確定時の結果先読み通知（`onAnswersFixed`） |
| `Zukan` | 23区カード一覧 |
| `WardModal` | トップ内の区概要、レーダー、レーダー根拠の基本7指標、詳細導線 |
| `ResultPage` | 診断セッションの有無に応じた結果カード（入場演出・にてる度%のカウントアップ・紙吹雪を含む）、タイプ特徴、相性理由、アコーディオン、相性ランキング、共有導線の出し分け |
