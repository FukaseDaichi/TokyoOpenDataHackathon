...
結果ページは、区別OGP画像を主ビジュアルとして表示する。自分の診断結果では続けて「あなたのタイプ」（タイプ名・説明文・区レーダーに利用者ベクトルを `overlay` で重ねた表示と凡例）、「なぜ相性がいいの？」（5軸の比較バーで利用者と区の位置を2種のマーカーで示し、軸差0.5以下の一致軸をハイライト。一致軸1位のAI相性文を「AIによる相性解説」バッジ付きで表示）、「実はこんな街」（一致軸に対応する基本指標のデータカードと順位バッジ、一致軸にタグが合う政策1件と出典リンク、区詳細導線）を表示する。その後にAIによるキャラクター設定理由、区単体の通常レーダーと基本7指標、区詳細導線、X共有導線、結果区を除く相性のよい他区3件を表示する。共有着地ではタイプ・相性理由・実はこんな街・X共有導線・相性3件を出さず、診断開始導線を表示する。相性カードにも各区の `public/og/{slug}.jpg` を使う。X共有文にはキャラクターの一言と `#うちの区ちゃん`、`#都知事杯オープンデータハッカソン` を含める。
