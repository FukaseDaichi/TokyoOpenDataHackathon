...
レーダー値 `[-1, 1]` は描画半径 `[0, 1]` へ変換する。結果ページでは区キャラクター単体の通常レーダーを表示し、利用者ベクトルの破線は重ねない。

結果ページは、区別OGP画像を主ビジュアルとして表示する。自分の診断結果ではX共有導線を出し、その後にAIによるキャラクター設定理由、通常レーダーと基本7指標、区詳細導線、結果区を除く相性のよい他区3件を表示する。共有着地ではX共有導線と相性3件を出さず、診断開始導線を表示する。相性カードにも各区の `public/og/{slug}.jpg` を使う。X共有文にはキャラクターの一言と `#うちの区ちゃん`、`#都知事杯オープンデータハッカソン` を含める。
