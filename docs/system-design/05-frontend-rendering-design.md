...
| `fallback`（reduced motion / WebGL不可） | `WardMap2D`（SVGの2D羊皮紙地図、フォールバックだが情報量は3Dと等価） |
...
3Dマップは `src/data/geo.ts` の `loadWardGeo()` と `src/lib/geo.ts` の純関数を2Dマップと共有し、`WardMesh` は `THREE.ExtrudeGeometry` / `EdgesGeometry` を区ごとに生成してアンマウント時に `dispose()` する。

## 5. 区モーダルとレーダー

区モーダルは、背景クリックまたはEscapeで閉じる。動きが許可されている場合は表紙の開閉アニメーションを行い、`animationend` を受け取れない場合に備えてタイマーでも状態を進める。

レーダーは `Radar`（SVGの2D表示）に統一し、区モーダル、結果ページ、区詳細、シェアカードで共有する。モーダルのステータスは区詳細ページと同じ `StatBar`（`buildWardStats` で項目を共用）で表示する。
...
- `WardMap2D` は `role="img"` と `「東京23区の中の◯◯区の位置」` の代替ラベルを持つ（3D版に代替テキストはなく、フォールバック時にDOMへ差し替わる）。
