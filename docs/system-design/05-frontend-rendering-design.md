...
| `assets/og/{slug}.png`（AI作成OGP原本） | `public/og/{slug}.png` | 1200×630のPNGへ加工して配置（スクリプト合成しない） |
...
OGPは1200×630pxのPNGを23区分そろえる。原本は生成AIに依頼して作成し、`assets/og/{slug}.png` に置いてから1200×630へ加工して `public/og/{slug}.png` に配置する。区ごとの生成プロンプトは [docs/strategy/og-image-prompts.md](../strategy/og-image-prompts.md) にまとめている。OGPをコードで合成生成する仕組みは持たない。
