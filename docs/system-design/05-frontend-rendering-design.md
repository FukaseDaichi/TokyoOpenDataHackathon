...
結果ページは、区別OGP画像、X共有導線、AIによるキャラクター設定理由、通常レーダーと基本7指標、相性のよい他区3件、区詳細導線の順に表示する。相性カードにも各区の `public/og/{slug}.jpg` を使う。X共有文にはキャラクターの一言と `#うちの区ちゃん`、`#都知事杯オープンデータハッカソン` を含める。
