...
結果ページのヒーローは診断済みか未診断かで分岐する。診断済みユーザーには「結果カード」（`data-testid="result-card"`）を表示する。中身はカード上部の区別OGP画像（`public/og/{slug}.jpg`、1200×630の横長）、`similarityPercent(ranked[0].distance)` によるにてる度%、タイプ名、一致軸バッジ（`selectMatchedAxes` の選定軸のうち利用者と区の実差が0.5以下のものだけ、最大2個）で、テーマカラーでカード全体を縁取る。未診断の閲覧者には従来どおり区別OGP画像と「あなたも診断する」CTAを表示する。自分の診断結果では続けて「YOUR TYPE」セクション（見出しは「あなたと{区名}ちゃんの重なり」。タイプ名は結果カードに出るためここでは重複させず、説明文・区レーダーに利用者ベクトルを `overlay` で重ねた表示・凡例のみを表示）、「なぜ相性がいいの？」（5軸の比較バーで利用者と区の位置を2種のマーカーで示し、軸差0.5以下の一致軸をハイライト。一致軸1位のAI相性文を「AIによる相性解説」バッジ付きで表示）、「実はこんな街」（一致軸に対応する基本指標のデータカードと順位バッジ、一致軸にタグが合う政策1件と出典リンク、区詳細導線）を表示する。その後にAIによるキャラクター設定理由、区単体の通常レーダーと基本7指標、区詳細導線、結果区を除く相性のよい他区3件を表示する。共有着地ではYOUR TYPE・相性理由・実はこんな街・相性3件を出さず、診断開始導線を表示する。相性カードにも各区の `public/og/{slug}.jpg` を使う。

X共有は `src/ui/share.ts` の `xShareText` / `xShareUrl` / `xWeightedLength` が担う。診断済みは「診断したら{区名}ちゃんと にてる度{XX}% だった！タイプは「{タイプ名}」らしい」、未診断は従来のキャラクターの一言入り文面で、いずれも末尾に `#うちの区ちゃん`、`#都知事杯オープンデータハッカソン` の2つのハッシュタグを付ける。X（Twitter）の加重文字数（半角1単位・全角や絵文字2単位、URLは常に23単位）で280単位を超えないことをVitestで保証する（全23区×最長タイプ名×にてる度100%の最悪ケースを含む）。共有CTAは結果カード直下のボタンと、画面下部に `position: fixed` で追従するシェアバー（診断済みのときだけ表示）の2箇所に置き、旧来の最下部単独シェア枠は廃止した。
