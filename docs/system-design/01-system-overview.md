...
「うちの区ちゃん」は、東京23区のオープンデータを5軸の性格ベクトルへ変換し、10問の回答から距離上位5区以内の区を提示する日本語のWebアプリケーションである。診断結果は全23区の回答パターン比率が1%以上10%以下になるようビルド時に校正する。診断、23区図鑑、区詳細、共有用の診断結果ページを提供する。
