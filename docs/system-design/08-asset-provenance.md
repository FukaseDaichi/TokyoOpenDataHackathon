...
| `assets/magic-circle.png` | 生成AIで作成。生成手順は未整理 | 要確認（プロンプト未記録） | 要確認（生成ツール・操作担当者未整理） | 要確認（生成ツールの利用規約、生成時プラン、入力素材の権利） | 生成日: 要確認 / 2026-07-12（リポジトリ初回取込日） | `scripts/build-modal-images.mjs`で幅1024px、PNG品質82・圧縮レベル9の`public/magic-circle.png`へ出力。現行UIからは未参照 |
...
| `data/raw/chika_r7_heikin.pdf` | 「令和7年地価公示 区市町村別用途別平均価格表」。住宅地・商業地の区市町村別平均。現行ジェネレーターでは利用せず、地点別CSVの検算用 | https://www.mlit.go.jp/totikensangyo/totikensangyo_fr4_000444.html | 国土交通省（PDFの直接配布組織は要確認） | 国土交通省ウェブサイト利用規約に従う。直接配布元の条件は要確認 | 2026-07-11（リポジトリ初回取込日） | 加工なし。現行ジェネレーターでは未使用 |
| `data/raw/estat_9-1-1.csv` | e-Statから財政力指数の別経路として取得を試行したファイル。実体は404 HTMLであり、対象表・利用列は要確認 | 要確認（現ファイルはe-Statの「ページが見つかりません」HTML） | 総務省統計局 / e-Stat | 政府統計の総合窓口の利用規約に従う。404 HTMLの再利用条件は要確認 | 2026-07-11（リポジトリ初回取込日） | 加工なし。現行ジェネレーターでは未使用 |
...
| `data/raw/metro_kt20tv0009.csv` | 東京都の統計から財政力指数の別経路として取得を試行したファイル。実体は404 HTMLであり、対象表・利用列は要確認 | 要確認（現ファイルは東京都の統計の「ページが見つかりません」HTML） | 東京都総務局統計部 | 東京都オープンデータカタログ由来はCC BY 4.0原則。ただし404 HTMLの再利用条件は要確認 | 2026-07-11（リポジトリ初回取込日） | 加工なし。現行ジェネレーターでは未使用 |
| `data/raw/soumu_000983090.xlsx` | 総務省「道府県庁所在地の主要財政指標」（令和5年度）。対象シート: `道府県庁所在地`。現行ジェネレーターで利用する列なし | 要確認 | 総務省 | 総務省ウェブサイト利用規約に従う | 2026-07-11（リポジトリ初回取込日） | 加工なし。主要財政指標一式として保存するが現行ジェネレーターでは未使用 |
| `data/raw/soumu_000983093.xlsx` | 総務省「全都道府県の主要財政指標」（令和5年度）。対象シート: `全都道府県の主要財政指標`。現行ジェネレーターで利用する列なし | 要確認 | 総務省 | 総務省ウェブサイト利用規約に従う | 2026-07-11（リポジトリ初回取込日） | 加工なし。主要財政指標一式として保存するが現行ジェネレーターでは未使用 |
...
| `data/raw/soumu_000983095.xlsx` | 総務省「市町村主要財政指標の都道府県別平均」（令和5年度）。対象シート: `都道府県別平均`。現行ジェネレーターで利用する列なし | 要確認 | 総務省 | 総務省ウェブサイト利用規約に従う | 2026-07-11（リポジトリ初回取込日） | 加工なし。主要財政指標一式として保存するが現行ジェネレーターでは未使用 |
| `data/raw/soumu_000983096.xlsx` | 総務省「政令指定都市の主要財政指標」（令和5年度）。対象シート: `政令指定都市`。現行ジェネレーターで利用する列なし | 要確認 | 総務省 | 総務省ウェブサイト利用規約に従う | 2026-07-11（リポジトリ初回取込日） | 加工なし。主要財政指標一式として保存するが現行ジェネレーターでは未使用 |
