...
| P3 | k-meansの収束判定がない | `src/lib/cluster.ts` は先頭6区を初期重心にして固定20回更新し、収束判定を行わない。表示名は `src/data/wards.ts` の `nameGroups` が代表区コード基準で街の雰囲気名（「ど真ん中シティ系」等）を割り当て、代表区の衝突・欠落は例外で検出する。現行23区の所属は `src/data/wards.test.ts` のスナップショットテストで固定済み | データや区順序の更新で所属が変わるとテストが落ちて検知できるため影響は限定的。ただし20回で未収束のまま打ち切られる可能性は残る | 収束判定（重心移動が閾値未満で停止）を導入し、所属変動時の命名見直し手順を運用に組み込む |
...
- 区章SVG 23枚は2026-07-31に削除した。著作権はPublic domainだったが、新宿区・練馬区の紋章使用承認要綱、荒川区の届出要綱などにより23区一律に適法と確認できないためである（[来歴台帳](08-asset-provenance.md) 2章）。代替の区アイコンは2026-07-31に23区分を生成済みで、原本 `assets/icons/{slug}.png` と配信用 `public/icons/{slug}.webp` がそれぞれ23件そろっている（[来歴台帳](08-asset-provenance.md) 3.3）。区詳細ページは読み込めた画像だけを表示する実装のままなので、原本が欠けても他の内容には影響しない。
